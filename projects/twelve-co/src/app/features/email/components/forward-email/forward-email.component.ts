...
export class ForwardEmailComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    constructor(
        ...
        private matDialogRef: MatDialogRef<ForwardEmailComponent>,
    ...
    ngOnInit(): void {
        ...
        this.subSink.sink = this.emailService.getEmailForId(this.preVal.lastEmailInChainId).subscribe(email => {
            ...
            const toList = this.getReceiversNameListAsString(this.lastEmailInChain.receivers);
            const ccList = this.getReceiversNameListAsString(this.lastEmailInChain.receiversCc);
            this.newBoxEmailItem.subject = this.lastEmailInChain.subject;
            const replayPrefix1 = `<p><br></p><p>---------- Forwarded message ---------</p>`;
            const replayPrefix2 = `<p>From: ${this.lastEmailInChain.createdBy?.name} &lt${this.lastEmailInChain.createdBy?.email}&gt </p>`;
            const replayPrefix3 = `<p>Date: ${new Date(this.lastEmailInChain.created).toLocaleString()}</p>`;
            const replayPrefix4= `<p>Subject: ${this.lastEmailInChain.subject}</p>`;
            const replayPrefix5= `<p>To: ${toList}</p>`;
            const replayPrefix6= `<p>Cc: ${ccList}</p>`;
            const replayPrefix = `${replayPrefix1}${replayPrefix2}${replayPrefix3}${replayPrefix4}${replayPrefix5}${replayPrefix6.length > 11 ? replayPrefix6 : ''}<p><br></p>`
    ...
    getReceiversNameListAsString( receivers: Array<Receiver>): string {
        let receiversString = '';
        for (let index = 0; index < receivers.length; index++) {
            const e = receivers[index];
            receiversString = receiversString + (index > 0 ? ', ' : '') + (e.accountName !== undefined ? e.accountName! : e.email!);
        }
